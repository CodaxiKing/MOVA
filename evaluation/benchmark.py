...
from evaluation.identity import IDENTITY_VERSION, analyze_reference, identity_metrics, load_embedder
from evaluation.motion import METRIC_VERSION, compare_tracks, load_tracks
from evaluation.temporal import TEMPORAL_VERSION, temporal_metrics
...
def evaluator_signature(protocol, identity_embedder=None):
    ...
    return fingerprint({"models": models, "code": code, "metric_version": METRIC_VERSION,
                        "identity_version": IDENTITY_VERSION, "temporal_version": TEMPORAL_VERSION,
                        "identity_embedder": identity_embedder, "protocol": protocol, "packages": {
...
def evaluate_benchmark(lock_path, outputs, *, label, generation_contract, artifacts_root="outputs/evaluation",
                       generation_index=None, resume_from=None, build_review_page=True, identity_embedder=None):
    ...
    signature = evaluator_signature(p, identity_embedder)  # Requires local extractor weights; never downloads here.
    embedder, embedder_error = load_embedder(identity_embedder)
    ...
    report = {"schema_version": 1, "label": label, "benchmark_sha256": locked["benchmark_sha256"],
              ...
              "protocol": p, "identity_embedder": identity_embedder, "identity_embedder_error": embedder_error,
              "cases": [], "quality_status": "NOT_ESTABLISHED", "status": "in_progress"}
    ...
    try:
        for case in locked["cases"]:
            ...
            try:
                ...
                generated_tracks = load_tracks(root / "generated")
                item["metrics"] = compare_tracks(load_tracks(root / "driver"), generated_tracks,
                                                 pck_threshold=p["pck_threshold"], visibility=p["visibility"])
                item["metrics"].update(frame_metrics(case, video, generated_tracks, embedder, p["visibility"]))
...
def frame_metrics(case, video, generated_tracks, embedder=None, visibility=0.5):
    """Identity (reference image vs generated frames) and temporal consistency (optical flow)."""
    import numpy as np
    ...
    with Image.open(resolve_path(case["reference"])) as im:
        reference = np.asarray(im.convert("RGB"))  # full resolution: better detection; metrics are scale-free
    ref_face, ref_body = analyze_reference(reference)
    face, body = generated_tracks["face"], generated_tracks["body"]
    identity = identity_metrics(reference, ref_face, ref_body, frames,
                                {"landmarks": np.asarray(face["landmarks"]), "present": np.asarray(face["present"])},
                                {"kp2d": np.asarray(body["kp2d"]), "present": np.asarray(body["present"])},
                                embedder=embedder, visibility=visibility)
    temporal = temporal_metrics(frames, read_video(resolve_path(case["motion"])))
    return {"identity": identity, "temporal": temporal}
...
COMPARED_FIELDS = {
    ...
    "identity": ("face_coverage", "face_geometry_error.mean", "face_color_delta_e.mean",
                 "face_color_hist_intersection.mean", "torso_color_delta_e.mean", "embedding.mean"),
    "temporal": ("generated.warp_error", "generated.static_flicker", "generated.luma_flicker",
                 "generated.mean_flow_px", "warp_error_ratio", "mean_flow_px_ratio"),
}


def metric_value(metrics, group, field):
    """Dotted lookup: metric_value(m, "identity", "face_geometry_error.mean"); None when absent."""
...
def compare_reports(baseline, candidate):
    ...
    for cid in sorted(before):
        ...
        for group, fields in COMPARED_FIELDS.items():
            for field in fields:
                a = metric_value(before[cid]["metrics"], group, field)
                b = metric_value(after[cid]["metrics"], group, field)
