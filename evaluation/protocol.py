...
INTAKE_FIELDS = ("identity_id", "reference_source", "motion_source", "reference_license", "motion_license", "prompt")


def write_manifest(path, manifest):
    """Rewrite a manifest keeping its leading comment block (the rest is regenerated by PyYAML)."""
...
def update_case(manifest, case_id, **fields):
    unknown = set(fields) - set(INTAKE_FIELDS)
    if unknown:
        raise ValueError(f"Unknown intake fields: {sorted(unknown)}")
    for case in manifest["cases"]:
        if case["id"] == case_id:
            for key, value in fields.items():
                if value is not None:
                    ...
                    case[key] = value
            return case
    raise ValueError(f"No case {case_id!r} in the manifest")
